...
# ═════════════════════════════════════════════════════════════════════
# DÉFINITION DES ONGLETS (Ordre modifié)
# ═════════════════════════════════════════════════════════════════════

tab_planning, tab_capa, tab_cong, tab_time, tab_active = st.tabs([
    "📋 Planning & ETA",  # <-- Premier onglet demandé
    "📊 Capacités",
    "🏖️ Congés & Run",
    "📈 Timeline Globale",
    "✅ En cours"
...
# ═════════════════════════════════════════════════════════════════════
# TAB 1: PLANNING & ETA (LOGIQUE PRINCIPALE)
# ═════════════════════════════════════════════════════════════════════
with tab_planning:
    ...
    if not df_plan.empty:
        ...
        # Conversion stricte en datetime pour éviter les erreurs
    ...
    if selected_project == "Vue Globale (Édition)":
        ...
        # --- MODE PROJET SPÉCIFIQUE (AVEC GANTT AMÉLIORÉ) ---
        ...
        if not df_filtered.empty:
            ...
            if not df_gantt.empty:
                ...
                fig = px.timeline(
                    ...
                    height=300 + (len(df_gantt) * 30)
                )
                
                # --- Configuration Avancée du Gantt ---
                
                # 1. Ajouter les zones d'Itérations (Fond de couleur)
                colors = ["rgba(200, 200, 200, 0.1)", "rgba(200, 200, 255, 0.1)", "rgba(200, 255, 200, 0.1)"]
                for i, it in enumerate(ITERATIONS):
                    # Zone de fond
                    fig.add_vrect(
                        x0=it["start"], x1=it["end"],
                        fillcolor=colors[i % len(colors)], layer="below", line_width=0,
                        annotation_text=it["name"], annotation_position="top left",
                        annotation_font_size=12, annotation_font_color="gray"
                    )
                
                # 2. Configuration de l'axe X (Jour par jour)
                fig.update_xaxes(
                    tickformat="%d/%m",  # Format JJ/MM
                    dtick=86400000.0,    # Un tick toutes les 24h (en ms)
                    ticklabelmode="period",
                    gridcolor="lightgray",
                    showgrid=True,
                    side="top"           # Labels en haut pour être proche des noms d'itération
                )
...
st.divider()
st.markdown(f"🛠 **PI Planning Tool v2.6** | Dernière mise à jour: {datetime.now().strftime('%d/%m/%Y %H:%M')}")
